...
//        $data['user_id'] = $request->external_source ? auth()->user()->id : $request->user_id;
        ...
        if (is_array($event["dates"]) || is_object($event["dates"])) {
            ...
            foreach ($event['dates']["items"]  as $date) {
                ...
                } else if (Carbon::parse(Carbon::parse($date['date'])->toDateString() . ' ' . $date['time']['from'])->lt(Carbon::now()->setSecond(0)) && Carbon::parse(Carbon::parse($date['date'])->toDateString() . ' ' . $date['time']['to'])->lt(Carbon::now()->setSecond(0))) {
