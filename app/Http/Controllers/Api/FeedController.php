...
class FeedController extends Controller
{
    ...
    public function allFeed(Request $request)
    {
        ...
        if ($subscriptions) {
            $entities = Timeline::where(function ($query) use ($subscriptions){
    ...
    public function getNewsFeed(Request $request)
    {
        ...
        if ($subscriptions) {
            $posts = Timeline::where('timelines_type','App\Models\Post')->where(function ($query) use ($subscriptions){
                foreach ($subscriptions as $ids) {
                    $query->OrWhere(function ($query) use ($ids){
                        $query->whereIn('user_id', $ids);
                    });
    ...
    public function getEventsFeed(Request $request)
    {
        ...
        if ($subscriptions) {
            $events = Timeline::where('timelines_type','App\Models\Event')->where(function ($query) use ($subscriptions){
                foreach ($subscriptions as $ids) {
                    $query->OrWhere(function ($query) use ($ids){
                        $query->whereIn('user_id', $ids);
                    });
