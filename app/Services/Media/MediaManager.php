...
class MediaManager
{
    ...
    public function processDeleted($model, $request, $collectionName = '')
    {
    ...
    public function save(Model $model, array $attrs, string $collectionName)
    {
        ...
        $customProperties['title'] = isset($attrs['title']) ? $attrs['title'] : '';
        $customProperties['alt'] = isset($attrs['alt']) ? $attrs['alt'] : '';
    ...
    {   /** @var Media $media */
        if ($media = Media::when($collectionName, function ($q) use ($collectionName) {
