...
class EventBegin extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function via($notifiable)
    {
        ...
        if ($notifiable->settings['event']['begin']['email'] ?? false) {
            $channels[] = 'mail';
        };

        if ($notifiable->settings['event']['begin']['site'] ?? false) {
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)
            ...
            ->greeting(trans('notification.event.begin'))
    ...
    public function sendPushNotification($notifiable)
    {
        ...
        $data = [
            ...
            "notification" => [
                "title" => trans('notification.event.begin') . ' - ' . $this->event->title,
                "body" => strip_tags($this->event->descr),
                "icon" => $this->event->getAllConversions()['avatar'],
                "action" => $url,
