...
class ChangeStatus extends Notification
{
     ...
     * @return void
     */
    public function __construct(Event $event, $item)
    {
        $this->event = $event;
        $this->item = $item;
    }
...
//        if (1) {
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)
            ->view(
                'emails.notifications.event.status',
                ['event' => $this->event,'user'=> $notifiable]
            )
            ->greeting(trans('notification.event.status.title'))
    ...
    public function sendPushNotification($notifiable)
    {
        ...
        $data = [
            ...
            "notification" => [
                "title" => trans('notification.event.status.title'),
                "body" => trans('notification.event.status.body'),
